...
#define ElementPos(y) 7+19*y
#define POSCheckBox(y) 7+17*y
...
template <typename F> struct Vec2_tpl {
	...
	ILINE Vec2_tpl(type_min);
	ILINE Vec2_tpl(type_max);
	explicit ILINE Vec2_tpl(F f) : x(f), y(f) { }
	ILINE Vec2_tpl() {
		Vec2_tpl(0, 0);
	}
	ILINE Vec2_tpl(F xt, F yt) {
		x = xt;
		y = yt;
	}
};
...
template <typename F> struct Quat_tpl {
	...
	/*
	ILINE F operator|(const Quat_tpl<F>& q, const Quat_tpl<F>& p)
	{
		CRY_MATH_ASSERT(q.v.IsValid());
		CRY_MATH_ASSERT(p.v.IsValid());
		return (q.v.x * p.v.x + q.v.y * p.v.y + q.v.z * p.v.z + q.w * p.w);
	}
	ILINE friend Quat_tpl<F> operator-(const Quat_tpl<F>& q, const Quat_tpl<F>& p)
	{
		return Quat_tpl<F1>(q.w - p.w, q.v - p.v);
	}
	ILINE friend void operator-=(Quat_tpl<F>& q, const Quat_tpl<F>& p)
	{
		q.w -= p.w;
		q.v -= p.v;
	}
	*/
	...
	ILINE Quat_tpl<F> operator-() const { Vec3 temp; temp.x = -v.x; temp.y = -v.y; temp.z = -v.z; return Quat_tpl<F>(-w, v); };
...
template<> inline Color_tpl<f32>::Color_tpl(f32 _x, f32 _y, f32 _z, f32 _w)
{
	r = _x; g = _y; b = _z; a = _w;
}
template<> inline Color_tpl<f32>::Color_tpl(f32 _x, f32 _y, f32 _z)
{
	r = _x; g = _y; b = _z; a = 1.f;
}
template<> inline Color_tpl<uint8>::Color_tpl(uint8 _x, uint8 _y, uint8 _z, uint8 _w)
{
	r = _x; g = _y; b = _z; a = _w;
}
template<> inline Color_tpl<uint8>::Color_tpl(uint8 _x, uint8 _y, uint8 _z)
{
	r = _x; g = _y; b = _z; a = 255;
}
template<> inline Color_tpl<f32>::Color_tpl(const unsigned int abgr)
{
	r = (abgr & 0xff) / 255.0f;
...
template<> inline Color_tpl<uint8>::Color_tpl(const unsigned int c) { *(unsigned int*)(&r) = c; }
template<> inline Color_tpl<f32>::Color_tpl(const float c)
{
	r = c;	g = c;	b = c;	a = c;
}
template<> inline Color_tpl<uint8>::Color_tpl(const float c)
{
	r = (uint8)(c * 255);	g = (uint8)(c * 255);	b = (uint8)(c * 255);	a = (uint8)(c * 255);
}
template<> inline Color_tpl<f32>::Color_tpl(const ColorF& c)
{
	r = c.r;	g = c.g;	b = c.b;	a = c.a;
}
template<> inline Color_tpl<uint8>::Color_tpl(const ColorF& c) {
	r = (uint8)(c.r * 255);	g = (uint8)(c.g * 255);	b = (uint8)(c.b * 255);	a = (uint8)(c.a * 255);
}
template<> inline Color_tpl<f32>::Color_tpl(const Vec3& c, float fAlpha)
{
	r = c.x;	g = c.y;	b = c.z;	a = fAlpha;
}
template<> inline Color_tpl<uint8>::Color_tpl(const Vec3& c, float fAlpha)
{
...
struct STextDrawContext
{
	...
	int m_drawTextFlags;
	STextDrawContext() : m_fxIdx(0), m_clipX(0), m_clipY(0), m_clipWidth(0), m_clipHeight(0), m_proportional(true), m_sizeIn800x600(true), m_clippingEnabled(false), m_framed(false), m_colorOverride(0, 0, 0, 0) {}
};
class Color
{
public:
	...
	inline f32 rBase() const { return this->r / 255.0f; }
	inline f32 gBase() const { return this->g / 255.0f; }
	inline f32 bBase() const { return this->b / 255.0f; }
	inline f32 aBase() const { return this->a / 255.0f; }
...
template <typename T> 
T CallFunction(DWORD64 dwLocation) { return (T)(dwLocation); }
